...
def test_update_requires_python(pyproject_requires_python: Pyproject) -> None:
...
@mark.parametrize("pyproject_requires_python", (">=3.11.10,<3.12",), indirect=True)
def test_update_requires_python_restritive(
    pyproject_requires_python: Pyproject,
) -> None:
    pyproject = update_requires_python(pyproject_requires_python)
    requires_python = SpecifierSet(pyproject.project_table["requires-python"])

    assert Version("3.11.9") not in requires_python
